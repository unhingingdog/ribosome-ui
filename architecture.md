...
sequenceDiagram
  ...
  Engine->>Engine: create_runtime config
  Engine->>Prompt: create_llm_prompt templates goal None
  ...
  Engine->>Engine: append raw assistant JSON to history
...
sequenceDiagram
  ...
  Engine->>Engine: serialize submission_payload JSON
  Engine->>Prompt: create_llm_prompt templates goal (Some submissionJson)
  Note over Engine,Prompt: every turn repeats schema and output contract;\nsubmission JSON is the current interaction
...
`Engine.ml` owns conversation history. Assistant turns currently store the raw accumulated generated template JSON from Telomere. There is a code TODO to revisit compaction or processing for prompt-engineering quality.
...
Creating the engine starts the initial UI turn from `goal_prompt`. User interaction proceeds through rendered UI submit callbacks owned by the library. The only current external lifecycle control is reset.
...
`processor_state.buffer` is the raw accumulated model output. `Engine.ml` currently stores that buffer as assistant history after a successful turn.
...
`Completion suffix` means the current buffer can be made valid JSON by appending `suffix`. `EngineBackend.handle_chunk` parses `processor_state.buffer ^ suffix` into a Ribosome template and renders it immediately if parsing succeeds.
...
- parser success becomes `Parsed`.
- parser failure or Telomere corruption becomes `Failed`.
