...
let is_non_string_data (c : char) (state : json_state) : bool =
  ...
  | Bracket (Empty | ExpectingValue) -> is_non_string_start c
  | Brace (InValue (NonString _))
  | Bracket (InValue (NonString _)) -> true
  | _ -> false
...
let parse_non_string_data (c : char) (state : json_state) :
  ...
  | Brace (InValue (NonString (Completable buffer | NonCompletable buffer))) ->
    continue_non_string c buffer (fun new_ns -> Brace (InValue (NonString new_ns)))
  | Bracket (InValue (NonString (Completable buffer | NonCompletable buffer))) ->
    continue_non_string c buffer (fun new_ns -> Bracket (InValue (NonString new_ns)))
