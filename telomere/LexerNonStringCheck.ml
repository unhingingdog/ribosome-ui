...
let check_incomplete_number_suffix (new_value : string) :
  ...
  if (last = 'e' || last = 'E')
     && (not has_exp)
     && float_of_string_opt prefix <> None then
    Ok Incomplete
  else if (last = '+' || last = '-')
          && String.length prefix > 0
          && (prefix.[String.length prefix - 1] = 'e'
              || prefix.[String.length prefix - 1] = 'E') then begin
...
let check (c : char) (buffer : string) :
  ...
  in
  if first_char = 't' || first_char = 'f' || first_char = 'n' then begin
    if List.mem new_value literals then
      Ok Complete
    else if List.exists (fun lit -> String.starts_with ~prefix:new_value lit) literals then
      Ok Incomplete
    else
      Error InvalidCharInLiteral
  end else if (first_char >= '0' && first_char <= '9') || first_char = '-' then begin
    if new_value = "-" then Ok Incomplete
    else
      ...
      | None ->
        check_incomplete_number_suffix new_value
  end else
    Error InvalidNonStringDataFirstChar
