...
let parse_quote_char (state : json_state) :
  ...
  match state with
  | Brace (Empty | ExpectingKey) -> Ok (OpenKey, Brace (InKey Open))
  | Brace ExpectingValue -> Ok (OpenStringData, Brace (InValue (String Open)))
  | Bracket (Empty | ExpectingValue) ->
      Ok (OpenStringData, Bracket (InValue (String Open)))
  | Brace (InKey Open) -> Ok (CloseKey, Brace (InKey Closed))
  | Brace (InKey Escaped) -> Ok (StringContent, Brace (InKey Open))
  | Brace (InKey Closed) -> Error QuoteCharAfterKeyClose
  ...
  | Brace (InValue (String Closed)) -> Error QuoteCharAfterValueClose
  ...
  | Bracket (InValue (String Closed)) -> Error QuoteCharAfterValueClose
