...
let feed (ps : processor_state) (chunk : string) : output * processor_state =
  ...
  Debug.log "processor" (Printf.sprintf "feed chunk_len=%d buffer_len=%d" (String.length chunk) (String.length new_buffer));
  match process_delta ps.balancer chunk with
  | Ok (completion, new_balancer) ->
      Debug.log "processor" (Printf.sprintf "completion suffix_len=%d" (String.length completion));
