...
let apply_event t ~event_id ~base_revision event : (event_result, string) result
  ...
  if base_revision <> t.revision then begin
    Debug.log "session"
      (Printf.sprintf
         "apply_event REJECT stale event_id=%s expected_rev=%d got_rev=%d"
         event_id t.revision base_revision);
    Error
    ...
    Debug.log "session"
      (Printf.sprintf "apply_event REJECT duplicate event_id=%s" event_id);
    ...
    | Some tree -> (
        ...
        | Ok (new_tree, _) -> (
            ...
            Debug.log "session"
              (Printf.sprintf "apply_event OK event_id=%s new_rev=%d" event_id
                 t.revision);
            match event with
...
let start_generation t ~gen_id =
  Debug.log "session"
    (Printf.sprintf "start_generation gen_id=%s session=%s" gen_id t.id);
...
let feed_delta t ~gen_id ~seq ~delta =
  ...
  | Some gen when seq <> gen.next_seq ->
      Debug.log "session"
        (Printf.sprintf
           "feed_delta REJECT out-of-order gen=%s expected_seq=%d got_seq=%d"
           gen_id gen.next_seq seq);
      Error
  ...
  | Some gen ->
      ...
      let t =
        ...
        | Incremental.Updated tree ->
            Debug.log "session"
              (Printf.sprintf "feed_delta updated gen=%s seq=%d new_rev=%d"
                 gen_id seq (t.revision + 1));
...
let complete_generation t ~gen_id =
  Debug.log "session"
    (Printf.sprintf "complete_generation gen_id=%s session=%s" gen_id t.id);
  match t.generation with
  | None -> Error "no active generation"
  | Some gen when gen.id <> gen_id -> Error "wrong generation"
...
let fail_generation t ~gen_id =
  Debug.log "session"
    (Printf.sprintf "fail_generation gen_id=%s session=%s" gen_id t.id);
  match t.generation with
  | None -> Error "no active generation"
  | Some gen when gen.id <> gen_id -> Error "wrong generation"
  | Some _ -> Ok { t with generation = None }
